...
def generate_decision_graph_text(data_path: str = None, available_hours: float = 2.0, target_mode: str = 'A++', target_subject: str = None, exam_scope_codes: list = None) -> str:
    ...
    roi_df = analyzer.module_d_priority_score(mode=target_mode, personal_modifiers=modifiers, target_subject=target_subject, exam_scope_codes=exam_scope_codes)
    trap_df = analyzer.module_b_trap_analysis(target_subject=target_subject, exam_scope_codes=exam_scope_codes)
...
def generate_decision_graph(data_path: str = None, output_path: str = "outputs/daily_suggestion.txt", available_hours: float = 2.0, target_mode: str = 'A++', target_subject: str = None, exam_scope_codes: list = None):
    """Legacy wrapper for terminal execution."""
    print(f"Initializing Phase 2 dynamic EDS Analyzer")
    output_text = generate_decision_graph_text(None, available_hours, target_mode, target_subject, exam_scope_codes)
